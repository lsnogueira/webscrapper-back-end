...
export class CagedProvider {
  ...
  public async cagedPage(): Promise<void> {
    mainLogin(this.page).then(this.irPaginaCaged, () =>
      console.error('Erro de login')
    );
  }

  private async carregarLabelTxtTelas(page: puppetteer.Page) {
    return async function carregandoElemento(elemento: string) {
      await page.waitForSelector('#' + elemento);
    };
  }

  private async irPaginaCaged(page: puppetteer.Page): Promise<void> {
    ...
    Object.keys(listaValoresElement).forEach(await this.carregarLabelTxtTelas(page));
